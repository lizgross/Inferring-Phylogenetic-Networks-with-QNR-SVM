...
Bio_Data_to_Vector <- function(h, structure = "vector"){
# 
#   Branch1 <- h[1,]
#   Branch2 <- h[2,]
#   Branch3 <- h[3,]
...
#   for (i in 1:dim(V)[1]){
#     for (j in 1:dim(V)[2]){
#       if (Branch[i,j] == 136) V[i,j] <- 1
#       else if (Branch[i,j] == 72) V[i,j] <- 2
#       else if (Branch[i,j] == 40) V[i,j] <- 3
#       else if (Branch[i,j] == 24) V[i,j] <- 4
#     }
#   }
# 
# 
#   Data_Tensor <- array(0,rep(4,length(Branch1)))
# 
#   for (k in 1:dim(V)[2]){
...
V <- rep(1,dim(Branch)[2])
primes<-c(2,3,5,7)
for (j in 1:dim(Branch)[2]){
  for (i in 1:4){
    if (Branch[i,j] == 136)     V[j] <- V[j]*primes[i]^1
    else if (Branch[[i,j]] == 72) V[j] <- V[j]*primes[i]^2
    else if (Branch[[i,j]] == 40) V[j] <- V[j]*primes[i]^3
    else if (Branch[[i,j]] == 24) V[j] <- V[j]*primes[i]^4
  }
}
...
Chp <- rep(1,dim(Ch)[2])
for (i in 1:dim(Ch)[2]){
  Chp[i]<-2^Ch[1,i]*3^Ch[2,i]*5^Ch[3,i]*7^Ch[4,i]
}
...
# by a tree 12|34 with random branch lengths lengths
...
# S3Cycle_Sampler: Function that returns the length 256 probability vector generated
# by the 3-cycle at far left in the poset (structure 4)
                            ...
                            , sites = 100, gamma1=0.5,gamma2 = "dummy")
{
  if (is.null(lengths) == T)
  {
    lengths <- c(runif(5, mn, mx/2), runif(3, mn, mx))
  } 
...
# S4Cycle_Sampler: Function that returns the length 256 probability vector 
...
# Two3Cycle_Sampler: Function that returns the length 256 probability vector
# generated by the double 3-cycle 12|34 (structure 22)
...
# SampleStructure: Randomly samples from a tree, 3-cycle, 4-cycle, or Two 3-cycle
...
#Classify_point: Implements the first and second layers of the decision algorithm.
# Input: A single data point (length 15 vector of Q coordinates), a KNN training set,
# and the SVM model for each of the four unlabeled level-one network structures
# Output: Classification of one of the 24 labeled level-one networks

Classify_point <- function(data_point
                           , neighbors = 3
                           , knn.train
                           , svm.fit.Tree
                           , svm.fit.3Cycle
  ...
  data_point_Inv <- ScoreAllInv_Sym(data_point)
  data_point_Inv <- t(as.matrix(data_point_Inv))

  colnames(data_point_Inv) <- {}

  NetworkStructure <- knn(train = knn.train[,-16]
                          , test = data_point
                          , cl = knn.train[,16]
                          , k = neighbors)

  if(NetworkStructure == 1){
    temp <- predict(svm.fit.Tree, data_point_Inv)
    return(as.numeric(temp))
  }

  else if(NetworkStructure == 2){
    temp <- predict(svm.fit.3Cycle, data_point_Inv)
    return(as.numeric(temp) + 3)}

  else if(NetworkStructure == 3){
    temp <- predict(svm.fit.4Cycle, data_point_Inv)
    return(as.numeric(temp) + 9)
  }

  else{
    temp <- predict(svm.fit.Two3Cycle, data_point_Inv)
    return(as.numeric(temp) + 21)
  }
}
